...
export function SceneLighting() {
  return (
    <>
      ...
      <directionalLight
        position={[8,12,5]} intensity={1.2} color="#FFE0B2"
        castShadow shadow-mapSize-width={2048} shadow-mapSize-height={2048}
        shadow-camera-far={50} shadow-camera-left={-15} shadow-camera-right={15}
        shadow-camera-top={15} shadow-camera-bottom={-15}
      />
      <pointLight position={[-5,6,-3]} intensity={0.4} color="#BBDEFB" />
      <pointLight position={[4,2,6]} intensity={0.3} color="#FFE082" />
  ...
  return (
    <>
      <mesh scale={[100,100,100]}>
        ...
        <meshBasicMaterial side={THREE.BackSide} color="#87CEEB" toneMapped={false} />
      </mesh>
      <mesh position={[0,15,-30]}>
        <planeGeometry args={[120,60]} />
        <meshBasicMaterial color="#FFE4C4" transparent opacity={0.4} toneMapped={false} />
      </mesh>
    </>
  );
}